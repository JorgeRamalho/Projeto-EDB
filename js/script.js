 ...
 * (cópia idêntica à raiz: /script.js)
 */
(function () {
  'use strict';
  ...
  function validate(values) {
    ...
    else if (values.nome.trim().length < 3) errors.nome = 'Nome deve ter pelo menos 3 caracteres.';

    if (!values.dataNascimento) errors.dataNascimento = 'Informe sua data de nascimento.';
    else {
    ...
    if (!phoneDigits) errors.telefone = 'Informe seu telefone.';
    ...
    else if (!emailRe.test(values.email.trim())) errors.email = 'E-mail inválido.';

    if (!values.endereco.trim()) errors.endereco = 'Informe seu endereço completo.';
  ...
  function initForm() {
    ...
    form.querySelectorAll('input').forEach((input) => {
      input.addEventListener('blur', () => {
        const errors = validate(getFormValues(form));
